#!/bin/sh
##SIMPLE FALLBACK ADAPTIVE LINK FOR APFPV, WORK ON KWAD SIDE, YOU CAN USE PIXELPILOT ANDROID OR SBC GS
# CONFIGURATION
...
while true; do
    ...
    elif [ "$bitrate" -lt "$bitratemax" ]; then
        bitrate=$bitratemax
        echo "Signal OK (dbm=$dbm), setting bitrate to $bitrate Mbps"
    fi

    # Set bitrate (convert Mbps to Kbps)
    wget -q -O /dev/null "http://localhost/api/v1/set?video0.bitrate=$((bitrate * 1024))"
done
